...
// Tra le varie alternative trovate sul web ho scelto la funzione Math.round che restituisce il valore del numero decimale approssimato all'intero più vicino.

function valToRound(value) {
  let rounded = Math.round(value);
  return console.log(rounded);
}

valToRound(7.84565758595959595050505);
...
// Primo tenativo di risoluzione, problema: non sono riuscito ad inserire gli elementi un array

function extractobj(i) {
  let productTitle = obj[i].title;
  let productPrice = obj[i].price;
  let productCount = obj[i].rating.count;
...
extractobj(0), extractobj(1), extractobj(2), extractobj(3), extractobj(4);

console.log("-----------------------");

// Secondo tenativo di risoluzione, problema: ho inserito gli elementi in un array, ma l'ordine non è corretto.
...
function extraction(obj) {
  ...
  for (let i = 0; i < 5; i++) {
    console.log(obj[i].rating.count);
  }
}

const objectExtracted = [];

objectExtracted.push(extraction(obj));

console.log(objectExtracted);
