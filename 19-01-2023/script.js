...
const selectCategory = document.querySelector("#choosecat");
const sendCategory = document.querySelector("#buttoncat");
...
//sendCategory.addEventListener("click", () => {
fetch("https://api.escuelajs.co/api/v1/products/")
  .then((res) => res.json())
  .then((data) => {
    const newData = data.filter(
      (item) => item.category.id === parseInt(promptInput)
    );
...
// Ho provato a inserire un eventlistener collegato al submit e a una selct ma non ha funzionato
console.log(selectCategory.value);

sendCategory.addEventListener("submit", (event) => {
  event.preventDefault();
  console.log(parseInt(selectCategory.value));
